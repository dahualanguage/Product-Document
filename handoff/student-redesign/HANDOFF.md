...
> 📄 **看實際長相：[`wg-goal-variants.html`](wg-goal-variants.html)** —— 八張卡並排，含三型的進行中／已達標，加上「沒有指派」、完整文案總表（含 i18n 變數），以及到期／取代規則。**橫幅文案本身分鐘型以外都是提案，尚未定案。**
...
- **過了 Due 但沒達標 → 橫幅維持原樣**（不變灰、不變紅、不收起來），直到**新目標指派下來才被取代**。沒有「已逾期」的專屬樣式。老師端「一班同一時間只有一個作業」是硬規則，所以卡片上永遠只有一塊橫幅。（2026-08-13 定案）
...
有設計疑問或不清楚的互動規則，請找設計者確認，不要自行臆測。特別是兩件**還沒定案**的事：計時器是否只在 min/day 目標時出現、以及學生端等級鎖定要如何對齊老師端的 `Coming soon`。（目標到期後的呈現已於 2026-08-13 定案，見「重點一」。）