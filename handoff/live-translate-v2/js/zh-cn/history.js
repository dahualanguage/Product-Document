/* ════════════════════════════════════════════════
   ...
   依赖：data.js, app.js, wordbank.js (buildWordCardEl)
   ════════════════════════════════════════════════ */
...
function buildCalendarDesktop() {
  ...
  var html = '<div class="cal-header">' +
    ...
    '<div class="cal-title">' + calYear + '年 ' + months[calMonth] + '</div>' +
...
function renderDesktopSessions(dateStr, sessions) {
  ...
  var html = '<div class="hist-day-label">' + label + ' — <span>' + sessions.length + '</span> 笔纪录</div>';

  if (!sessions.length) {
    html += '<div class="hist-day-empty">这天没有录音纪录</div>';
...
function renderMobileSessions(dateStr, sessions) {
  ...
  if (!sessions.length) {
    section.innerHTML = '<div class="hist-day-empty">这天没有录音纪录</div>';
    return;
  }
  ...
  sessions.forEach(function(session) {
    ...
    card.innerHTML =
      ...
      '<div class="hist-s-meta">' +
        '<div class="hist-s-meta-item"><span class="material-icons-round">access_time</span>' + session.time + '</div>' +
        '<div class="hist-s-meta-item"><span class="material-icons-round">timer</span>' + session.duration + '</div>' +
        '<div class="hist-s-meta-item"><span class="material-icons-round">bookmark</span>' + wordCount + ' 个单字</div>' +
      '</div>' +
  ...
  right.innerHTML =
    '<div class="detail-meta-card">' +
      ...
      '<div class="detail-meta-row">' +
        ...
        '<div class="detail-meta-pill"><span class="material-icons-round">bookmark</span> ' + session.words.length + ' 个单字</div>' +
        '<div class="detail-lang-dot">' + langDisplay + '</div>' +
        '<div style="margin-left:auto;display:flex;align-items:center;gap:8px;">' +
          '<span style="font-size:12px;font-weight:500;color:#64748b;">翻译</span>' +
  ...
  if (session.summary) {
    ...
    card.innerHTML =
      '<div class="ai-summary-header">' +
        '<div class="ai-summary-badge"><span class="material-icons-round">auto_awesome</span> AI 摘要</div>' +
      '</div>' +
...
function buildHistWords(scrollId, session) {
  ...
  if (!session.words.length) {
    scroll.innerHTML = '<div style="text-align:center;padding:32px;color:#94a3b8;font-size:13px;">本次未保存任何单字</div>';
    return;
  }
...
function buildDetailWords(scrollId, session) {
  ...
  scroll.innerHTML = '';
  if (!session.words.length) {
    scroll.innerHTML = '<div class="wd-empty"><span class="material-icons-round">bookmark_border</span>本次未保存任何单字</div>';
    return;
  }
  session.words.forEach(function(w) {
    scroll.appendChild(buildWordCardEl(w, session.lang));
