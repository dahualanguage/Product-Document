...
function buildWordCardEl(w, lang) {
  ...
  var headHtml =
    '<div class="wd-head">' +
      ...
      '</div>' +
      '<button class="wd-play" onclick="event.stopPropagation();showToast(\'🔊 ' + w + '\')" title="发音">' +
...
function buildWordBankDesktop(filtered, freq) {
  var allWords = getAllUniqueWords();
  var sortLabel = wbSortNewest ? '最新↓' : '最旧↓';
...
function renderWbWordList() {
  ...
  if (!pageWords.length) {
    container.innerHTML = '<div style="text-align:center;padding:32px 0;color:#94a3b8;font-size:13px;">没有符合的单字</div>';
...
function buildWordBankMobile(filtered, freq) {
  ...
  if (stats) stats.innerHTML = '词汇数：<span>' + filtered.length + '</span>';

  /* Sort link */
  var sortLink = document.getElementById('wb-sort-link-mobile');
  if (sortLink) sortLink.textContent = wbSortNewest ? '最新↓' : '最旧↓';
...
function renderWbMobileCards(filtered, freq) {
  ...
  if (!filtered.length) {
    scroll.innerHTML = '<div class="wd-empty"><span class="material-icons-round">bookmark_border</span>没有符合的单字</div>';
    return;
  }
