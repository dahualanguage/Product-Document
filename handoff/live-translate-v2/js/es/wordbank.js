...
function buildWordCardEl(w, lang) {
  ...
  var headHtml =
    '<div class="wd-head">' +
      ...
      '</div>' +
      '<button class="wd-play" onclick="event.stopPropagation();showToast(\'🔊 ' + w + '\')" title="Pronunciación">' +
...
function buildWordBankDesktop(filtered, freq) {
  var allWords = getAllUniqueWords();
  var sortLabel = wbSortNewest ? 'Más recientes ↓' : 'Más antiguas ↓';
  ...
  if (filters) {
    filters.innerHTML =
      ...
      '<button class="wb-filter-chip' + (wbFilter === 'freq' ? ' active' : '') + '" onclick="filterWb(\'freq\')">Más guardadas</button>' +
...
function renderWbWordList() {
  ...
  if (!pageWords.length) {
    container.innerHTML = '<div style="text-align:center;padding:32px 0;color:#94a3b8;font-size:13px;">Sin palabras coincidentes</div>';
...
function renderWbDetailCard(word) {
  ...
  html += '<span class="wb-card-play" onclick="showToast(\'🔊 ' + word + '\')" title="Reproducir"><svg viewBox="0 0 24 24"><path d="M3 9v6h4l5 5V4L7 9H3zm13.5 3A4.5 4.5 0 0 0 14 8.5v7a4.49 4.49 0 0 0 2.5-3.5zM14 3.23v2.06a6.51 6.51 0 0 1 0 13.42v2.06A8.51 8.51 0 0 0 14 3.23z"/></svg></span>';
...
function buildWordBankMobile(filtered, freq) {
  ...
  if (filters) {
    filters.innerHTML =
      '<button class="wb-filter-chip' + (wbFilter === 'all' ? ' active' : '') + '" onclick="filterWb(\'all\')">Todas</button>' +
      '<button class="wb-filter-chip' + (wbFilter === 'freq' ? ' active' : '') + '" onclick="filterWb(\'freq\')">Más guardadas</button>';
  }

  /* Stats */
  var stats = document.getElementById('wb-stats-mobile');
  if (stats) stats.innerHTML = 'Palabras: <span>' + filtered.length + '</span>';

  /* Sort link */
  var sortLink = document.getElementById('wb-sort-link-mobile');
  if (sortLink) sortLink.textContent = wbSortNewest ? 'Más recientes ↓' : 'Más antiguas ↓';
...
function renderWbMobileCards(filtered, freq) {
  ...
  if (!filtered.length) {
    scroll.innerHTML = '<div class="wd-empty"><span class="material-icons-round">bookmark_border</span>Sin palabras coincidentes</div>';
    return;
  }
  ...
  if (wbViewMode === 'list') {
    ...
    filtered.forEach(function(w) {
      ...
      item.innerHTML =
        ...
        '<button class="wd-list-play" onclick="event.stopPropagation();showToast(\'🔊 ' + w + '\')" title="Reproducir">' +
          '<span class="material-icons-round">volume_up</span>' +
        '</button>';
